...
class AppPrefs(private val sharedPreferences: SharedPreferences) {
    ...
    inner class Internal : ManagedPreferenceInternal(sharedPreferences) {
        val pinyinModeRime = string("input_method_pinyin_mode_rime", CustomConstant.SCHEMA_ZH_T9)
        val inputDefaultMode = int("input_default_method_mode", InputModeSwitcherManager.MODE_T9_CHINESE)
        val inputMethodPinyinMode = int("input_method_pinyin_mode", InputModeSwitcherManager.MODE_T9_CHINESE)
        val dataDictVersion = int("rime_dict_data_version", 0)
        val keyboardHeightRatio = float("keyboard_height_ratio", 0.3f)
        val keyboardHeightRatioLandscape = float("keyboard_height_ratio_landscape", 0.5f)
        val candidatesHeightRatio = float("candidates_height_ratio", 0.035f)
        val candidatesHeightRatioLandscape = float("candidates_height_ratio_landscape", 0.06f)
        val keyboardModeFloat = bool("keyboard_mode_float", false)
        val keyboardModeFloatLandscape = bool("keyboard_mode_float_landscape", false)// 悬浮模式:横屏
        val keyboardBottomPaddingFloat = int("keyboard_padding_bottom", DevicesUtils.dip2px(100))
        val keyboardRightPaddingFloat = int("keyboard_padding_right", DevicesUtils.dip2px(20))
        val keyboardBottomPaddingLandscapeFloat = int("keyboard_padding_bottom_landscape", DevicesUtils.dip2px(50))
        val keyboardRightPaddingLandscapeFloat = int("keyboard_padding_right_landscape", DevicesUtils.dip2px(20))
        val keyboardBottomPadding = int("keyboard_padding_bottom_normal", DevicesUtils.dip2px(0))
        val keyboardRightPadding = int("keyboard_padding_right_normal", DevicesUtils.dip2px(0))
        val clipboardUpdateTime = long("clipboard_update_time", 0)
        val clipboardUpdateContent = string("clipboard_update_content","")
        val fullDisplayKeyboardEnable = bool("full_display_keyboard_enable", true)
        val fullDisplayKeyModeLeft = string("full_display_key_mode_left", FullDisplayKeyMode.SwitchIme.name)
        val fullDisplayKeyModeRight = string("full_display_key_mode_right", FullDisplayKeyMode.Clipboard.name)
        val fullDisplayCenterMode = string("full_display_center_mode", FullDisplayCenterMode.MoveCursor.name)

        val soundOnKeyPress = int("key_press_vibration_amplitude",0)
        val vibrationAmplitude = int("key_press_sound_volume", 0)

        val privacyPolicySure = bool("privacy_policy_sure", false)
    }
