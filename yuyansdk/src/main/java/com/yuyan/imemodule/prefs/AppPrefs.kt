...
class AppPrefs(private val sharedPreferences: SharedPreferences) {
    ...
    inner class Internal : ManagedPreferenceInternal(sharedPreferences) {
        ...
        val voiceModelId = string("voice_model_id", "builtin_zh")  // 语音识别模型ID（仅内部存储）
    ...
    inner class Voice : ManagedPreferenceCategory(R.string.setting_ime_input, sharedPreferences) {
        val modelManagement = custom(
            R.string.voice_model_management,
    ...
    inner class WebDAV : ManagedPreferenceInternal(sharedPreferences) {
        val enabled = bool("webdav_enabled", false)
        val serverUrl = string("webdav_server_url", "")
        val username = string("webdav_username", "")
        val password = string("webdav_password", "")
        val remotePath = string("webdav_remote_path", "/yuyan_ime_backup/")
        val autoSync = bool("webdav_auto_sync", false)
        val syncOnWifiOnly = bool("webdav_sync_wifi_only", true)
        val ignoreSSLCert = bool("webdav_ignore_ssl_cert", false)
        val lastSyncTime = long("webdav_last_sync_time", 0L)
        val lastSyncSuccess = bool("webdav_last_sync_success", false)
    }
    ...
    fun syncToDeviceEncryptedStorage() {
        ...
        sp.edit {
            ...
            webdav.managedPreferences.forEach {
                it.value.putValueTo(this@edit)
            }
        }
    }
