...
abstract class ManagedPreferenceUi<T : Preference>(
    ...
    class EditTextString(
        ...
        override fun createUi(context: Context) = EditTextPreference(context).apply {
            ...
            if (this@EditTextString.summary != null) {
                setSummary(this@EditTextString.summary)
            }
        }
    }
    ...
    class Preference(
        ...
        val summary: Int? = null,
        enableUiOn: (() -> Boolean)? = null
    ) : ManagedPreferenceUi<androidx.preference.Preference>(key, enableUiOn) {
        override fun createUi(context: Context) = androidx.preference.Preference(context).apply {
            key = this@Preference.key
            isIconSpaceReserved = false
            isSingleLineTitle = false
            setTitle(this@Preference.title)
            if (this@Preference.summary != null) {
                setSummary(this@Preference.summary)
            }
        }
    }
}