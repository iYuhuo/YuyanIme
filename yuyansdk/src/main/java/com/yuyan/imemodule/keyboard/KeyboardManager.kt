...
import com.yuyan.imemodule.keyboard.container.VoiceInputContainer
...
class KeyboardManager {
    enum class KeyboardType {
        T9, QWERTY, LX17, QWERTYABC, NUMBER, SYMBOL, SETTINGS, HANDWRITING, CANDIDATES, ClipBoard, TEXTEDIT, VOICE
    }
    ...
    fun switchKeyboard(keyboardName: KeyboardType) {
        ...
        // 如果当前是语音输入界面，切换到其他界面时清理资源
        if (currentContainer is VoiceInputContainer && keyboardName != KeyboardType.VOICE) {
            (currentContainer as VoiceInputContainer).hideVoiceInputView()
        }
        ...
        if (container == null) {
            container = when (keyboardName) {
                ...
                KeyboardType.VOICE -> VoiceInputContainer(Launcher.instance.context, mInputView)
