...
fun onSettingsMenuClick(inputView: InputView, skbMenuMode: SkbMenuMode) {
    when (skbMenuMode) {
        ...
        SkbMenuMode.VoiceInput -> {
            // 切换到语音输入界面
            if (KeyboardManager.instance.currentContainer is VoiceInputContainer) {
                // 如果已经在语音输入界面，则返回到普通键盘
                KeyboardManager.instance.switchKeyboard()
            } else {
                // 切换到语音输入界面
                KeyboardManager.instance.switchKeyboard(KeyboardManager.KeyboardType.VOICE)
            }
            inputView.updateCandidateBar()
        }
        else ->{}
