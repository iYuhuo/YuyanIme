...
class OtherSettingsFragment: ManagedPreferenceFragment(AppPrefs.getInstance().other){
    ...
    override fun onPreferenceUiCreated(screen: PreferenceScreen) {
        ...
        // Rime词库学习（提升词频）
        screen.addPreference("词库学习（提升词频）") {
            findNavController().navigate(R.id.action_otherSettingsFragment_to_rimeDictImportFragment)
        }
        
        // WebDAV 同步（仅在线版本）
        if (!BuildConfig.offline) {
            screen.addPreference("☁️ WebDAV 云同步") {
                findNavController().navigate(R.id.action_otherSettingsFragment_to_webdavSettingsFragment)
            }
        }
