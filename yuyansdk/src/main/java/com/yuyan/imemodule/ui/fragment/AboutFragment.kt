    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        preferenceScreen = preferenceManager.createPreferenceScreen(requireContext()).apply {
            ...
            addCategory(R.string.app_version) {
                ...
                addPreference(R.string.build_git_hash, 
                    if (BuildConfig.AppCommitHead.isNotEmpty()) BuildConfig.AppCommitHead else "未知") {
                    if (BuildConfig.AppCommitHead.isNotEmpty() && BuildConfig.AppCommitHead != "unknown") {
                        val commit = BuildConfig.AppCommitHead.substringBefore('-')
                        val uri = Uri.parse("${CustomConstant.YUYAN_IME_REPO}/commit/${commit}")
                        startActivity(Intent(Intent.ACTION_VIEW, uri))
                    }
                }
                addPreference(R.string.build_time, 
                    if (BuildConfig.AppBuildTime.isNotEmpty()) BuildConfig.AppBuildTime else "未知")
