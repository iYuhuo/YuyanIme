...
open class BaseKeyboardView(mContext: Context?) : View(mContext) {
    ...
    private fun dispatchGestureEvent(downEvent: MotionEvent?, currentEvent: MotionEvent, distanceX: Float, distanceY: Float) : Boolean {
        ...
        if(lastEventX < 0 || lastEventActionIndex != currentEvent.actionIndex) {
            lastEventX = currentX
            lastEventY = currentY
        ...
        val isVertical = relDiffX * 1.5 < relDiffY
        ...
        if (!isVertical && relDiffX > spaceSwipeMoveCursorSpeed) {
            ...
            } else if (isSwipeKey && AppPrefs.getInstance().keyboardSetting.spaceSwipeMoveCursor.getValue()) {
                removeMessages()
                lastEventX = currentX
                lastEventY = currentY
                mAbortKey = true
            ...
            if (isVertical && distanceY > 0 && relDiffY > symbolSlideUp && ThemeManager.prefs.keyboardSymbol.getValue()){
                lastEventX = currentX
                lastEventY = currentY
                lastEventActionIndex = currentEvent.actionIndex
                mLongPressKey = true
            ...
            if (isVertical && relDiffY > symbolSlideUp * 2) {
                lastEventX = currentX
                lastEventY = currentY
                lastEventActionIndex = currentEvent.actionIndex
                mLongPressKey = true
